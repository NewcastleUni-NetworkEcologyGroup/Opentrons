...
def run(protocol: protocol_api.ProtocolContext):
    ...
    tiprack_10a = protocol.load_labware('opentrons_96_filtertiprack_20ul', 10)
    tiprack_10b = protocol.load_labware('opentrons_96_filtertiprack_20ul', 11)
    ...
    pipette_multi10 = protocol.load_instrument('p10_multi', mount='right', tip_racks=[tiprack_10a,tiprack_10b]) 
    ...
    for d in dest_plates:
        pipette_multi10.pick_up_tip()
        print(pipette_multi10.well_bottom_clearance.aspirate)
        pipette_multi10.transfer(PCR_matermix_vol,mastermix,d.rows_by_name()['A'],
                                touch_tip=True,
                                new_tip='never',
                                blow_out=True,
                                blow_out_location='source well')
        pipette_multi10.well_bottom_clearance.aspirate = round(pipette_multi10.well_bottom_clearance.aspirate-(pipette_multi10.well_bottom_clearance.aspirate/steps))
        pipette_multi10.drop_tip()        

    # reset the set the p10multi aspirate height to something sensible for a primer plate
