...
def run(protocol: protocol_api.ProtocolContext):
    ...
    available_slots = [1,4] # this order minimises pipette travel over non-target wells
    number_of_destination_plates: int = 2
    if number_of_destination_plates > 2:
        raise Exception('Please specify 2 or fewer destination plates, you dont have enough primer combinations')
    ...
    for ind, d in enumerate(dest_plates):
        ...
        pipette_300.distribute(PCR_mastermix_vol,mastermix,d.wells()[range(0,48)],
                                #touch_tip=True,
                                #radius=0.8,
                                new_tip='never',
                                blow_out=True,
