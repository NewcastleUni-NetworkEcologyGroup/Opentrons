...
Tris = tubes.wells('A2')
start_vol_SybrGreen: float = 5760
...
Tris_vols=[320,320,320,320,320,320,320]
...
pipette1000.distribute(100, SybrGreen.top(pipette_height_SybrGreen-5),
...
# Distribute TE to the sample wells to dilute the DNA samples later
pipette1000.distribute(95, Tris.top(-105),
                       Control_plate.wells('A7', to='H9'),
                       disposal_vol=30,
                       blow_out=SybrGreen,
                       touch_tip=True)
...
#set up some target lists
row = ['A','B','C','D','E','F','G','H']
col = ['1','2','3']

#use a nested loop to target the values of col for each row and distribute the control DNAs to the control wells
for y in row:
    pipette1000.pick_up_tip()
    for x in col:
        pipette1000.transfer(100, trough.well(y+'1').bottom(2),
                               Control_plate.well(y+x),
                               disposal_vol=10,
                               blow_out=trough.well(y+'1'),
                               touch_tip=True,
                               v_offset=-1,
                               new_tip='never')
    pipette1000.drop_tip()
